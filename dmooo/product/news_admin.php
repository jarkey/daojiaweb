...
<form action="" method="POST" name="aa">
  ...
  <tr>
    <td colspan="9" class="tdBorder1 titleBg1">商品列表</td>
    </tr>  
    ...
    <td class="tdBorder1 tdBg1">规格</td>
    <td class="tdBorder1 tdBg1">品牌</td>
...
?>
    ...
    <td class="tdBorder1 "><a href="news_img.php?class=<?=$id?>&fid=<?=$_GET['class']?>&classIndex=<?=$_GET['classIndex']?>">图片</a></td>
    <td class="tdBorder1 "><a href="news_price.php?class=<?=$id?>&fid=<?=$_GET['class']?>&classIndex=<?=$_GET['classIndex']?>">规格</a></td>
    <td class="tdBorder1 "><?=ppzView($data['brand'])?></td>
	<td class="tdBorder1"><?=$times?></td>
	<td class="tdBorder1">
	  <a href="news_revised.php?class=<?=$class?>&id=<?=$id?>&keepThis=true&TB_iframe=true&height=430&width=860" title="新闻管理 / 信息修改" class="thickbox"><img src="../images/xiug.gif" alt="修改" width="14" height="15" border="0"></a>&nbsp;&nbsp;
	  <?php 
	  	if ($date['issystem']!=1)
	{
		?>
